...
const { data, error } = await db
  .from("knowledge_gaps")
  .select("topic, gap_tickets, distinct_tickets, no_kb_source, weak_grounding, languages, recent_ticket_ids, recent_subjects, last_seen")
  .limit(limit);
...
console.log(`Top ${rows.length} knowledge gaps (by frequency) — ${total} ungrounded ticket(s) shown.\n`);
console.log("Read each as 'a KB article / operational note to write', not a grade of the AI.\n");

rows.forEach((r, i) => {
  const langs = Array.isArray(r.languages) ? r.languages.join("/") : "";
  const kind = r.no_kb_source ? `${r.no_kb_source} no-KB-source, ${r.weak_grounding} weak-grounding`
    : `${r.weak_grounding} weak-grounding`;
  console.log(
    `${String(i + 1).padStart(2)}. ${r.topic}  —  ${r.gap_tickets} gap(s) [${kind}]  ${langs}`,
  );
